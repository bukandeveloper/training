...
// explode url to get first folder
$explodeUrl	= explode('/', $url);
// get first folder from url
$folder 		= $explodeUrl[1];
...
$config 		= array(
	...
	*/
	...
	|
	|--------------------------------------------------------------------------
	| its also check if running on localhost or xserver
	|
	| if running localhost the (upload_dir => /dir/laravel/public/js/source/)
	| if running xserver the (uplaod_dir => /js/source/)
	--------------------------------------------------------------------------
	*/
	// 'upload_dir' => '/dir/laravel/public/js/source/',
	'upload_dir' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http"). "://". @$_SERVER['HTTP_HOST']=='http://localhost'?'/'.$folder.'/laravel/public/js/source/':'/js/source/',
	/*
	|--------------------------------------------------------------------------
